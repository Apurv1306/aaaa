...
# Global variable to hold the Flask subprocess
flask_process = None

def get_ip_addresses():
    """
    Retrieves the IP addresses of the machine.
    Attempts to get local IP and an external-facing IP.
    """
    ip_list = []
    try:
        ...
        # Attempt to get an external-facing IP by connecting to a public DNS server
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.settimeout(0)
        try:
            # Doesn't actually send data, just establishes connection to get local IP
            s.connect(("8.8.8.8", 80))
            ip_list.append(s.getsockname()[0])
        finally:
            s.close()
    except Exception:
        # Catch any exceptions during IP retrieval and ignore
        pass
    # Return unique IP addresses
    return list(set(ip_list))

class FlaskController(BoxLayout):
    """
    Kivy layout for controlling the Flask server.
    Contains a status label, a start/stop button, and an IP display label.
    """
    def __init__(self, **kwargs):
        """
        Initializes the FlaskController layout and its widgets.
        """
        super().__init__(orientation='vertical', **kwargs)
        
        # Label to display Flask server status
        self.status_label = Label(text='Flask is stopped', size_hint=(1, 0.2))
        
        # Button to start/stop the Flask server
        self.start_stop_button = Button(text='Start Flask', size_hint=(1, 0.2))
        self.start_stop_button.bind(on_press=self.toggle_flask) # Bind button press to toggle_flask method

        # Label to display Flask server IP addresses
        self.ip_label = Label(text='No IPs yet', size_hint=(1, 0.6))
    ...
    def toggle_flask(self, instance):
        """
        Toggles the Flask server on or off.
        Manages the subprocess and updates the UI labels.
        """
        global flask_process # Access the global flask_process variable

        if flask_process is None:
            # If Flask is not running, start it
            self.status_label.text = 'Starting Flask...'
            # Use subprocess.Popen to run the Flask app in a new process
            # sys.executable ensures the current Python interpreter is used
            # __file__ refers to the current script, and "runserver" is an argument
            # to trigger the Flask backend part
            flask_process = subprocess.Popen([sys.executable, __file__, "runserver"])
            self.start_stop_button.text = 'Stop Flask'
            self.status_label.text = 'Flask is running'
            
            # Get and display IP addresses
            ips = get_ip_addresses()
            self.ip_label.text = '\n'.join([f"http://{ip}:5000" for ip in ips])
        else:
            # If Flask is running, stop it
            self.status_label.text = 'Stopping Flask...'
            flask_process.terminate() # Terminate the Flask subprocess
            flask_process = None # Reset the global variable
            self.start_stop_button.text = 'Start Flask'
            self.status_label.text = 'Flask is stopped'
            self.ip_label.text = 'No IPs yet'
...
if __name__ == '__main__':
    # This block determines whether to run the Flask backend or the Kivy GUI
    # based on command-line arguments.
    if len(sys.argv) > 1 and sys.argv[1] == 'runserver':
        # If "runserver" argument is provided, run the Flask app
        app.run(host='0.0.0.0', port=5000, debug=True)
    else:
        # Otherwise, run the Kivy GUI application
        FlaskKivyApp().run()
